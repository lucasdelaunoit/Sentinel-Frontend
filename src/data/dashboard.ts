...
export interface RiskEvent {
  id: string;
  /** ISO date the disruption begins. */
  date: string;
  employee: string;
  initials: string;
  kind: RiskEventKind;
  /** Plain-language operational consequences. */
  impacts: string[];
  severity: RiskLevel;
}

export const RISK_EVENT_KIND_LABEL: Record<RiskEventKind, string> = {
  leave: "Planned leave",
  sick_leave: "Sick leave",
  resignation: "Resignation risk",
  overlap: "Overlapping absence",
  certification: "Certification expiry",
  shortage: "Staffing shortage",
};

export const UPCOMING_RISK_EVENTS: RiskEvent[] = [
  {
    id: "RE1",
    date: "2026-05-28",
    employee: "Sarah Chen",
    ...
    kind: "leave",
    impacts: [
      "Cloud Migration loses Kubernetes expertise",
      "Coverage drops to 62%",
      "Bus factor becomes 1",
    ],
    severity: "high",
  },
  {
    id: "RE2",
    date: "2026-05-30",
    employee: "Clint Cambier",
    initials: "CC",
    kind: "overlap",
    impacts: [
      "Overlaps with Sarah Chen absence",
      "Backend left without senior cover",
      "2 projects without redundancy",
    ],
    severity: "critical",
  },
  {
    ...
    kind: "certification",
    impacts: [
      "ISO 27001 audit lead unavailable",
      "Compliance review blocked",
    ],
    severity: "medium",
  },
  {
    ...
    kind: "leave",
    impacts: [
      "Internal Dev Platform loses Terraform owner",
      "Coverage drops to 55%",
    ],
    severity: "medium",
  },
];
