...
export default function CompanyHolidayDetailSheet({ holiday, open, onOpenChange }: CompanyHolidayDetailSheetProps) {
  ...
  const startFull = start.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" });
  const endFull = end.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" });
  const headlineDateLabel = sameDay ? startFull : `${startFull} → ${endFull}`;
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <span className="text-[24px] font-black leading-tight text-foreground">{holiday.name}</span>
              <span className="mt-1 text-[13px] font-medium text-muted-foreground">{headlineDateLabel}</span>
            </div>
          </div>
