...
const SKILL_CATEGORIES = ["FRONTEND", "BACKEND", "DEVOPS", "DATABASE", "SECURITY", "TESTING"] as const;
...
const DEFAULT_SKILLS: SkillDefinition[] = [
  ...
  { id: "s11", name: "Angular",           category: "FRONTEND",  description: "Angular framework",           criticality: "low",      minRedundancy: 1 },
  { id: "s12", name: "Next.js",           category: "FRONTEND",  description: "React SSR framework",         criticality: "high",     minRedundancy: 2 },
  { id: "s13", name: "TailwindCSS",       category: "FRONTEND",  description: "Utility CSS framework",       criticality: "medium",   minRedundancy: 2 },
  { id: "s14", name: "Figma",             category: "FRONTEND",  description: "UI/UX design tool",           criticality: "low",      minRedundancy: 1 },
  // BACKEND
  ...
  { id: "s15", name: "PHP / Laravel",     category: "BACKEND",   description: "Laravel framework",           criticality: "high",     minRedundancy: 2 },
  { id: "s16", name: "Go",               category: "BACKEND",   description: "Go language",                 criticality: "medium",   minRedundancy: 1 },
  { id: "s17", name: "GraphQL",           category: "BACKEND",   description: "GraphQL API design",          criticality: "medium",   minRedundancy: 2 },
  { id: "s18", name: "REST API Design",   category: "BACKEND",   description: "REST principles",             criticality: "high",     minRedundancy: 3 },
  // DEVOPS
  ...
  { id: "s19", name: "Docker",            category: "DEVOPS",    description: "Containerization",            criticality: "high",     minRedundancy: 3 },
  { id: "s20", name: "Terraform",         category: "DEVOPS",    description: "Infrastructure as code",      criticality: "medium",   minRedundancy: 2 },
  { id: "s21", name: "CI/CD Pipelines",   category: "DEVOPS",    description: "GitHub Actions / GitLab CI", criticality: "high",     minRedundancy: 2 },
  // DATABASE
  { id: "s8",  name: "PostgreSQL",        category: "DATABASE",  description: "PostgreSQL database",         criticality: "medium",   minRedundancy: 2 },
  { id: "s22", name: "MySQL",             category: "DATABASE",  description: "MySQL database",              criticality: "medium",   minRedundancy: 2 },
  { id: "s23", name: "MongoDB",           category: "DATABASE",  description: "NoSQL document store",        criticality: "low",      minRedundancy: 1 },
  { id: "s24", name: "Redis",             category: "DATABASE",  description: "In-memory cache",             criticality: "medium",   minRedundancy: 2 },
  // SECURITY
  { id: "s9",  name: "Security Eng.",     category: "SECURITY",  description: "Security architecture",       criticality: "critical", minRedundancy: 2 },
  { id: "s25", name: "Pen Testing",       category: "SECURITY",  description: "Penetration testing",         criticality: "high",     minRedundancy: 1 },
  { id: "s26", name: "OWASP / Compliance",category: "SECURITY",  description: "Regulatory compliance",       criticality: "critical", minRedundancy: 2 },
  // TESTING
  { id: "s10", name: "QA / Manual",       category: "TESTING",   description: "Manual QA",                  criticality: "low",      minRedundancy: 1 },
  { id: "s27", name: "Jest / Vitest",     category: "TESTING",   description: "Unit testing",               criticality: "medium",   minRedundancy: 2 },
  { id: "s28", name: "Cypress / Playwright", category: "TESTING", description: "E2E testing",               criticality: "medium",   minRedundancy: 2 },
  { id: "s29", name: "Load Testing",      category: "TESTING",   description: "k6 / Locust",                criticality: "low",      minRedundancy: 1 },
];
...
function SkillsTab({ skills: initialSkills, onSave }: { skills: SkillDefinition[]; onSave: (s: SkillDefinition[]) => void }) {
  ...
  const [newSkill, setNewSkill] = useState<Partial<SkillDefinition>>({
    name: "", category: "", description: "", criticality: "medium", minRedundancy: 2,
  });
  ...
  useEffect(() => { setPage(1); }, [search, critFilter, selectedCat]);
  ...
  const fieldCls = "w-full rounded-xl border border-border/60 bg-background px-4 py-2.5 text-[13px] text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/40 transition-all";
  ...
  return (
    <>
        ...
        <ComposedCard
          ...
          action={
            <div className="ml-auto flex items-center gap-2">
              <span className="text-[11px] font-medium text-muted-foreground/50 tabular-nums">{categories.length}/{MAX_CATEGORIES}</span>
              <Button
                onClick={() => setCatSheetOpen(true)}
                disabled={categories.length >= MAX_CATEGORIES}
                size="sm"
                variant="outline"
                className="h-7 px-2.5 rounded-lg text-[11px] gap-1 border-border/60"
              >
          ...
          <div className="flex flex-col mt-3" style={{ minHeight: "440px" }}>
            <button
              ...
              className={cn(
                ...
                selectedCat === "ALL" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted/50 hover:text-foreground",
              )}
            >
              All skills
              <span className={cn("text-[11px] font-bold tabular-nums", selectedCat === "ALL" ? "text-primary" : "text-muted-foreground/50")}>
                {list.length}
              </span>
            </button>
                  ...
                  return (
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        <p className={cn("text-[12px] font-semibold truncate", isActive ? "text-primary" : "text-foreground")}>{cat}</p>
                        <p className="text-[10px] text-muted-foreground/50">{count} skill{count !== 1 ? "s" : ""}</p>
                      </div>
                      {count === 0 ? (
                        <button
                        ...
                        <span className={cn("text-[10px] font-bold tabular-nums shrink-0", isActive ? "text-primary" : "text-muted-foreground/40")}>{count}</span>
                      )}
                    </div>
                  );
                })
            ...
            <div className="ml-3 flex items-center gap-2 flex-1">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full h-9 rounded-xl border border-border/60 bg-background pl-9 pr-3 text-[13px] text-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 transition-all"
                />
              </div>
              <select
                value={critFilter}
          ...
          <div className="mt-4 space-y-4">
            ...
            {hasFilter && (
              <div className="flex items-center gap-2">
                <span className="text-[12px] text-muted-foreground">{filtered.length} of {catSkillCount} skill{catSkillCount !== 1 ? "s" : ""}</span>
                <button onClick={() => { setSearch(""); setCritFilter("all"); }} className="text-[11px] text-primary hover:underline">
                  Clear filters
                </button>
              </div>
            )}
            ...
            {paginated.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-48 text-muted-foreground gap-2">
                ...
                {hasFilter && (
                  <button onClick={() => { setSearch(""); setCritFilter("all"); }} className="text-[12px] text-primary hover:underline">Clear filters</button>
                )}
              </div>
            ) : (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                          <span className={cn("inline-flex items-center rounded-full px-1.5 py-0.5 text-[9px] font-semibold", crit.pill)}>
                            {skill.criticality}
                          </span>
                          {selectedCat === "ALL" && (
                          ...
                          <span className="text-[9px] text-muted-foreground/40 ml-auto">Min ×{skill.minRedundancy}</span>
                        </div>
                      </div>
                    </div>
                  );
      ...
      <ComposedSheet
        ...
        footer={
          <>
            <Button variant="outline" onClick={() => setSkillSheetOpen(false)} className="flex-1 rounded-xl">Cancel</Button>
            <Button
      ...
      >
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-muted-foreground/70 uppercase tracking-wide">Skill Name</label>
            <input
              type="text"
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-muted-foreground/70 uppercase tracking-wide">Criticality</label>
            <select
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-muted-foreground/70 uppercase tracking-wide">Min Redundancy</label>
            <input
              type="number"
              min={1}
              max={10}
          ...
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-muted-foreground/70 uppercase tracking-wide">Description <span className="normal-case text-muted-foreground/50">(optional)</span></label>
            <input
              type="text"
      ...
      <ComposedSheet
        ...
        footer={
          <>
            <Button variant="outline" onClick={() => setCatSheetOpen(false)} className="flex-1 rounded-xl">Cancel</Button>
            <Button
              onClick={addCategory}
      ...
      >
        <div className="space-y-4">
          <div className="space-y-1.5">
            <label className="text-[11px] font-medium text-muted-foreground/70 uppercase tracking-wide">Category Name</label>
            <input
              type="text"
          ...
          <div className="rounded-xl bg-amber-50 border border-amber-200 px-4 py-3 flex items-start gap-2.5">
            ...
            <div>
              <p className="text-[12px] font-semibold text-amber-700">{categories.length}/{MAX_CATEGORIES} categories used</p>
              <p className="text-[11px] text-amber-600 mt-0.5">Radar charts become unreadable beyond 8 axes. This is the enforced maximum.</p>
            </div>
          </div>
          <div className="space-y-1.5">
            <p className="text-[11px] font-medium text-muted-foreground/70 uppercase tracking-wide">Existing categories</p>
            <div className="flex flex-wrap gap-1.5">
              {categories.map((cat) => (
                <span key={cat} className="text-[11px] font-semibold bg-muted/50 text-muted-foreground rounded-full px-2.5 py-1">
                  {cat}
                </span>
              ))}
            </div>
