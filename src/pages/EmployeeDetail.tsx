...
import { PenSquare, X, PlayCircle, Plus, ChevronRight, Calendar, Briefcase, Mail, Phone, User, CalendarDays } from 'lucide-react'
import { cn } from '@/lib/utils'
import { Button } from '@/components/ui/button'
import { EMPLOYEE_DETAILS, type EmployeeDetail, type LeaveType, type SkillCategory } from '@/data/employees'
...
const RADAR_AXES: SkillCategory[] = ['FRONTEND', 'BACKEND', 'DEVOPS', 'DATABASE', 'SECURITY', 'TESTING']
...
function LeaveBadge({ type }: { type: LeaveType }) {
  return (
    <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold capitalize', LEAVE_COLORS[type])}>
      {LEAVE_LABELS[type]}
    </span>
  )
}

function LeaveStatusBadge({ status }: { status: 'approved' | 'pending' | 'rejected' }) {
  const cls = {
    ...
    <div className="grid grid-cols-3 gap-4">
      ...
      <div className="rounded-2xl bg-card border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-foreground text-sm">Informations of {employee.name.split(' ')[0]}</h3>
          <Button
            size="xs"
            variant="outline"
        ...
        <div className="space-y-3">
          {[
            ...
            { icon: CalendarDays, label: 'Start Date', value: new Date(employee.startDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) },
      ...
      <div className="rounded-2xl bg-card border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-foreground text-sm">Leaves of {employee.name.split(' ')[0]}</h3>
          <Button
            size="xs"
            variant="outline"
            className="gap-1.5 text-[11px] rounded-lg"
        ...
        <div className="space-y-2">
          {employee.leaves.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">No leaves recorded</p>
          ) : (
      ...
      <div className="rounded-2xl bg-card border border-border p-5">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-foreground text-sm">Projects of {employee.name.split(' ')[0]}</h3>
          <Button
            size="xs"
            variant="outline"
            className="gap-1.5 text-[11px] rounded-lg"
  ...
  return (
    <div className="rounded-2xl bg-card border border-border overflow-hidden">
      <div className="px-6 py-4 border-b border-border flex items-center justify-between">
        ...
        <Button size="xs" variant="outline" className="gap-1.5 text-[11px] rounded-lg">
          <Plus className="size-3" />
          Assign project
        </Button>
      </div>
        ...
        <thead>
          <tr className="border-b border-border bg-muted/20">
            {['Project', 'Role', 'Status'].map((col) => (
              <th key={col} className="px-6 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">
                {col}
              </th>
            ))}
          </tr>
            ...
            <tr key={proj.id} className="hover:bg-muted/10 transition-colors">
              ...
              <td className="px-6 py-4">
                <span className={cn(
                  ...
                  proj.status === 'Active' ? 'bg-emerald-100 text-emerald-700' : 'bg-muted text-muted-foreground',
                )}>
                  {proj.status}
                </span>
              </td>
    ...
    <div className="grid grid-cols-5 gap-4">
      ...
      <div className="col-span-3 rounded-2xl bg-card border border-border p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button size="xs" variant="ghost" className="gap-1.5 text-xs rounded-lg text-muted-foreground hover:text-foreground">
...
export default function EmployeeDetail() {
  ...
  const employee: EmployeeDetail | undefined = id ? EMPLOYEE_DETAILS[id] : undefined

  if (!employee) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        <p className="text-lg font-semibold text-foreground">Employee not found</p>
        <Link to="/employees" className="text-sm text-primary underline underline-offset-4">
          Back to employees
        </Link>
      </div>
    )
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-1.5 text-sm text-muted-foreground mb-0.5">
              <Link to="/employees" className="hover:text-foreground transition-colors font-medium">
                Employee
              </Link>
              <ChevronRight className="size-3.5" />
              <span className="font-semibold text-foreground">{employee.name}</span>
            </div>
            <p className="text-xs text-muted-foreground">Get all information's about {employee.name}</p>
          </div>
          <Button
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <div className="rounded-2xl bg-card border border-border p-5">
            <div className="flex items-start justify-between">
              <p className="text-sm font-medium text-muted-foreground">Bus Factor in Organization</p>
              <button className="flex size-7 items-center justify-center rounded-lg bg-muted text-muted-foreground/50 hover:bg-muted/80 transition-colors">
                <Briefcase className="size-3.5" />
              </button>
            </div>
            <p className="mt-3 text-3xl font-bold text-amber-500">{employee.busFactor}</p>
          </div>
        </div>
